	...
	<div>
		<div class="center_view">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form">
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.yishenggonghao"
								placeholder="Doctor's ID number" clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.keguashuliangStart"
								placeholder="Minimum appointment quantity" clearable>
							</el-input>-
							<el-input class="search_inp" v-model="searchQuery.keguashuliangEnd"
								placeholder="Maximum appointment quantity" clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-select class="search_sel" clearable v-model="searchQuery.ispay"
								placeholder="state of payment">
								<el-option label="have paid" value="have paid"></el-option>
								<el-option label="non-payment" value="non-payment"></el-option>
							</el-select>
						</div>
					</div>
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('yuyueguahao', '新增')">
						add
					</el-button>
					<el-button class="del_btn" type="danger" :disabled="selRows.length ? false : true"
						@click="delClick(null)" v-if="btnAuth('yuyueguahao', '删除')">
						delete
					</el-button>
					<el-button class="statis_btn" type="warning" @click="echartClick1"
						v-if="btnAuth('yuyueguahao', '挂号数量')">
						The number of registrations
					</el-button>
				</div>
			</div>
			<el-table v-loading="listLoading" border :stripe='false' @selection-change="handleSelectionChange"
				ref="table" v-if="btnAuth('yuyueguahao', '查看')" :data="list" @row-click="listChange">
				...
				<el-table-column min-width="140" :resizable='true' :sortable='true' align="left" header-align="left"
					prop="yuyuebianhao" label="reservation number">
					<template #default="scope">
						{{ scope.row.yuyuebianhao }}
					</template>
				</el-table-column>
				<el-table-column min-width="140" :resizable='true' :sortable='true' align="left" header-align="left"
					prop="yishenggonghao" label="Doctor's ID number">
					<template #default="scope">
						{{ scope.row.yishenggonghao }}
					</template>
				</el-table-column>
				<el-table-column min-width="140" :resizable='true' :sortable='true' align="left" header-align="left"
					prop="yishengxingming" label="Doctor's Name">
					<template #default="scope">
						{{ scope.row.yishengxingming }}
					</template>
				</el-table-column>
				<el-table-column min-width="140" :resizable='true' :sortable='true' align="left" header-align="left"
					prop="guahaofei" label="Registration Fee">
					<template #default="scope">
						{{ scope.row.guahaofei }}
					</template>
				</el-table-column>
				<el-table-column min-width="140" :resizable='true' :sortable='true' align="left" header-align="left"
					prop="keguashuliang" label="Number of registrations">
					<template #default="scope">
						{{ scope.row.keguashuliang }}
					</template>
				</el-table-column>
				<el-table-column min-width="140" :resizable='true' :sortable='true' align="left" header-align="left"
					prop="yuyueshijian" label="appointment time">
					<template #default="scope">
				...
				<el-table-column prop="ispay" label="Payment Status" min-width="140" :resizable='true' :sortable='true'
					...
					<template #default="scope">
						<span style="margin-right:10px">{{ scope.row.ispay == 'have paid' ? 'have paid' : 'non-payment' }}</span>
						<el-button v-if="scope.row.ispay != 'have paid' && btnAuth('yuyueguahao', '支付')" size="small"
		...
		<el-dialog v-model="echartVisible" title="statistical chart" width="70%">
			<div id="keguashuliangEchart1" style="width:100%;height:600px;"></div>
			<template #footer>
...
const formName = 'appointment register'
...
const getList = () => {
	...
	params['sort'] = 'id'
	params['order'] = 'desc'
	if (searchQuery.value.yishenggonghao && searchQuery.value.yishenggonghao != '') {
		params['yishenggonghao'] = '%' + searchQuery.value.yishenggonghao + '%'
	}
	if (searchQuery.value.keguashuliangStart) {
		params['keguashuliangStart'] = searchQuery.value.keguashuliangStart
	}
	if (searchQuery.value.keguashuliangEnd) {
		params['keguashuliangEnd'] = searchQuery.value.keguashuliangEnd
	}
...
const delClick = (id) => {
	...
	ElMessageBox.confirm(`Do you want to delete the selected items ${formName}`, '提示', {
		confirmButtonText: '是',
		cancelButtonText: '否',
		type: 'warning',
	}).then(() => {
...
const echartClick1 = () => {
	...
	nextTick(() => {
		var keguashuliangEchart1 = echarts.init(document.getElementById("keguashuliangEchart1"), 'theme');
		context.$http({
			url: `${tableName}/value/yishenggonghao/keguashuliang?order=desc`,
			method: 'get'
		}).then(res => {
