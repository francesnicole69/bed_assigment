	...
	<div>
		<div class="center_view">
			<div class="list_search_view">
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('tijianxinxi', '新增')">
						add
					</el-button>
					<el-button class="del_btn" type="danger" :disabled="selRows.length ? false : true"
						@click="delClick(null)" v-if="btnAuth('tijianxinxi', '删除')">
						delete
					</el-button>
				</div>
			</div>
			<el-table v-loading="listLoading" border :stripe='false' @selection-change="handleSelectionChange"
				ref="table" v-if="btnAuth('tijianxinxi', '查看')" :data="list" @row-click="listChange">
				...
				<el-table-column min-width="140" :resizable='true' :sortable='true' align="left" header-align="left"
					prop="tijianshijian" label="physical examination time">
					<template #default="scope">
				...
				<el-table-column label="operation" width="300" :resizable='true' :sortable='true' align="left"
					header-align="left">
					<template #default="scope">
						<el-button class="view_btn" type="info" v-if="btnAuth('tijianxinxi', '查看')"
							@click="infoClick(scope.row.id)">
							details
						</el-button>
						<el-button class="edit_btn" type="primary" @click="editClick(scope.row.id)"
							v-if="btnAuth('tijianxinxi', '修改')">
							edit </el-button>
						<el-button class="del_btn" type="danger" @click="delClick(scope.row.id)"
							v-if="btnAuth('tijianxinxi', '删除')">
							delete </el-button>
						<el-button class="cross_btn" v-if="btnAuth('tijianxinxi', '体检报告')" type="success"
							@click="tijianbaogaoCrossAddOrUpdateHandler(scope.row, 'cross', '', '', '', '')">
		...
		<tijianbaogaoFormModel ref="tijianbaogaoFormModelRef" @formModelChange="formModelChange">
		</tijianbaogaoFormModel>
	</div>
</template>
<script setup>
...
const tijianbaogaoCrossAddOrUpdateHandler = (row, type, crossOptAudit, crossOptPay, statusColumnName, tips, statusColumnValue) => {
	...
	nextTick(() => {
		tijianbaogaoFormModelRef.value.init(row.id, 'cross', '体检报告', row, 'tijianxinxi', statusColumnName, tips, statusColumnValue)
	})
}
//初始化
const init = () => {
