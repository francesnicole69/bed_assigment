<template>
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search" @submit.native.prevent>
			<div class="search_view">
				...
				<div class="search_box">
					<el-input class="search_inp" v-model="searchQuery.yishengxingming" placeholder="Doctor's Name"
						clearable>
					</el-input>
				</div>
			</div>
			...
			<div class="search_view">
				...
				<div class="search_box">
					<el-select class="search_sel" clearable v-model="searchQuery.keshi" placeholder="Department">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('yishengzuozhen', '新增')"
		...
		<div class="category_view">
			...
			<div class="category" :class="categoryIndex == index ? 'categoryActive' : ''" v-for="(item, index) in categoryList"
				:key="index" @click="categoryClick(index)">{{ item }}</div>
		</div>
		<div class="page_list">
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item, index) in list" :key="index" @click.stop="detailClick(item.id)">
						<div class="data_img_box" v-if="item.touxiang && item.touxiang.substr(0, 4) == 'http'"
						...
						<div class="data_img_box" v-else
							@click.stop="preViewClick($config.url + item.touxiang.split(',')[0])">
							<el-image class="data_img" :src="item.touxiang ? $config.url + item.touxiang.split(',')[0] : ''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title1">
								<span>Doctor's Name:{{ item.yishengxingming }}</span>
							</div>
							<div class="data_title2">
								<span>{{ item.keshi }}</span>
							</div>
							<div class="data_title3">
								<span>registration fee:{{ item.guahaofei }}</span>
							</div>
							<div class="data_title4">
								<span>available slots:{{ item.keguashuliang }}</span>
							</div>
						</div>
					</div>
				</div>
...
const addClick = () => {
	router.push('/index/yishengzuozhenAdd')
}
//判断是否从个人中心跳转
...
const getList = () => {
	...
	if (categoryIndex.value != -1) {
		params.keshi = categoryList.value[categoryIndex.value]
	}
