	...
	<div>
		<div class="center_view">
			...
			<el-table v-loading="listLoading" border :stripe='false' @selection-change="handleSelectionChange"
				ref="table" v-if="btnAuth('config', '查看')" :data="list" @row-click="listChange">
				...
				<el-table-column label="value" min-width="140" width="120" :resizable='true' :sortable='true'
					align="left" header-align="left">
					<template #default="scope">
						<div v-if="scope.row.value">
							<el-image v-if="scope.row.value.substring(0, 4) == 'http'" preview-teleported
								:preview-src-list="[scope.row.value.split(',')[0]]" :src="scope.row.value.split(',')[0]"
