	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">Type of physical examination</div>
					<div  class="info_text" >{{detail.tijianleixing}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Physical examination location</div>
					<div  class="info_text" >{{detail.tijiandidian}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Physical examination cost</div>
					<div  class="info_text" >{{detail.tijianfeiyong}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Appointment count</div>
				...
				<div class="info_item">
					<div class="info_label">Institutional Account Number</div>
				...
				<div class="info_item">
					<div class="info_label">Physical examination status</div>
					<div  class="info_text" >{{detail.tijianzhuangtai}}</div>
				</div>
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('tijianyuyue','体检')" @click="tijianxinxionAcross('体检','是','否','tijianzhuangtai','已体检','已体检,未体检'.split(',')[0])" type="warning">体检</el-button>
					<el-button class="cross_btn" v-if="btnFrontAuth('tijianyuyue','取消')" @click="quxiaoyuyueonAcross('取消','否','否','','')" type="warning">cancel</el-button>
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('tijianyuyue','支付')" class="approval_btn" @click="payClick">pay</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('tijianyuyue','修改')" type="primary" @click="editClick">edit</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('tijianyuyue','删除')" type="danger" @click="delClick">delete</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="Appointment Details" name="first">
				<div v-html="detail.yuyuexiangqing"></div>
			</el-tab-pane>
		</el-tabs>
...
<script setup>
	...
	const tijianxinxionAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('Please log in before proceeding!','error')
			return false
		}
		if(!btnAuth('tijianyuyue',btnType)){
			context?.$toolUtil.message('No permission to operate!','error')
			return false
		}
        if(crossOptPay=='Yes'&&detail.value.ispay!='have paid') {
		...
		nextTick(()=>{
			router.push(`/index/tijianxinxiAdd?type=cross&&id=${detail.value.id}`)
		})
	}
