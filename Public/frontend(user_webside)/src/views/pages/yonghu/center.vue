	...
	<div class="center_view" :style='{}'>
		...
		<div class="usersView">
			<div class="usersTabView">
                ...
                <div class="usersTab" v-if="btnAuth('storeup','查看')" @click="tabClick({tableName:'storeup',type:1})">my favorite</div>
			</div>
			<div class="usersBox updateInfo" v-if="tabIndex=='center'">
				<el-form class="usersForm" ref="userFormRef" :model="userForm" label-width="120px" :rules="rules">
					<el-row>
						...
						<el-col :span="12">
							<el-form-item prop="zhuzhi" label="address">
								<el-input class="list_inp" v-model="userForm.zhuzhi" placeholder="address" ></el-input>
							</el-form-item>
						</el-col>
					</el-row>
			...
			<div class="usersBox updatePassword" v-if="tabIndex=='updatePassword'">
				<el-form class="usersForm" ref="passwordFormRef" :model="passwordForm" label-width="120px"
					...
					<div class="formModel_btn_box">
						<el-button class="formModel_confirm" @click="updatePassword">Update Password</el-button>
					</div>
				</el-form>
			</div>
		</div>
		...
		passwordFormRef.value.validate(async (valid) => {
			if (valid) {
				if(passwordForm.value.mima != userForm.value.yonghumima){
					context?.$toolUtil.message('Incorrect original password', 'error')
					return false
				}
                ...
                if(passwordForm.value.mima==passwordForm.value.newmima){
                    context?.$toolUtil.message('New password cannot be the same as the original password', 'error')
                    return false
                }
