	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">Physical Examination Type</div>
				...
				<div class="info_item">
					<div class="info_label">Examination Time</div>
					<div class="info_text">{{ detail.tijianshijian }}</div>
				</div>
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('tijianxinxi', '体检报告')"
						@click="tijianbaogaoonAcross('体检报告', '', '', '', '')" type="warning">medical examination
						report</el-button>
					<el-button class="edit_btn" v-if="centerType && btnAuth('tijianxinxi', '修改')" type="primary"
						@click="editClick">edit</el-button>
					<el-button class="del_btn" v-if="centerType && btnAuth('tijianxinxi', '删除')" type="danger"
						@click="delClick">delete</el-button>
				</div>
			</div>
		</div>
	</div>
...
const formName = 'Physical examination information'
//基础信息
const breadList = ref([{
	name: formName
}])
...
const tijianbaogaoonAcross = async (btnType, crossOptAudit, crossOptPay, statusColumnName, tips, statusColumnValue, type = 1) => {
	if (!context?.$toolUtil.storageGet('frontToken')) {
		context?.$toolUtil.message('Please log in before proceeding!', 'error')
		return false
	}
	if (!btnAuth('tijianxinxi', btnType)) {
	...
	nextTick(() => {
		router.push(`/index/tijianbaogaoAdd?type=cross&&id=${detail.value.id}`)
	})
}
onMounted(() => {
	init()
