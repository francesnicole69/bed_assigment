			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						<span>{{detail.yishengxingming}}</span>
					</div>
				</div>
				<div class="info_item">
					<div class="info_label">Doctor's employee number</div>
					<div  class="info_text" >{{detail.yishenggonghao}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					...
					<div  class="info_text" >{{detail.zhicheng}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">Specialized Field</div>
					<div  class="info_text" >{{detail.shanzhanglingyu}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">contact information</div>
					<div  class="info_text" >{{detail.lianxifangshi}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Consultation Time</div>
					<div  class="info_text" >{{detail.zuozhenshijian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">The quantity of items</div>
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('yishengzuozhen','挂号')" @click="yuyueguahaoonAcross('挂号','','','','')" type="warning">registration</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('yishengzuozhen','修改')" type="primary" @click="editClick">edit</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('yishengzuozhen','删除')" type="danger" @click="delClick">delete</el-button>
				</div>
			</div>
		</div>
	</div>
...
<script setup>
	...
	const yuyueguahaoonAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('Please log in before proceeding!','error')
			return false
		}
		if(!btnAuth('yishengzuozhen',btnType)){
		...
		nextTick(()=>{
			router.push(`/index/yuyueguahaoAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
