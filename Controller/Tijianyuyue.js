...
import TijianyuyueModel from '../models/TijianyuyueModel'
...
export default ({ config, db }) => {
	...
	const renameMap = {
	    ...
	    "ispay": "ispay",
	    "tijianzhuangtai": "tijianzhuangtai",
	    "yuyuexiangqing": "yuyuexiangqing",
	};
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let tijianzhuangtai = req.query.tijianzhuangtai
			if (tijianzhuangtai) {

				if (tijianzhuangtai.indexOf('%') != -1) {
					where.tijianzhuangtai = {
						[Op.like]: tijianzhuangtai
					}
				} else {
					where.tijianzhuangtai = {
						[Op.eq]: tijianzhuangtai
					}
				}
			}
			let yuyuexiangqing = req.query.yuyuexiangqing
			if (yuyuexiangqing) {

				if (yuyuexiangqing.indexOf('%') != -1) {
					where.yuyuexiangqing = {
						[Op.like]: yuyuexiangqing
					}
				} else {
					where.yuyuexiangqing = {
						[Op.eq]: yuyuexiangqing
					}
				}
			}
			...
			let result = await TijianyuyueModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await TijianyuyueModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await TijianyuyueModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await TijianyuyueModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await TijianyuyueModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await TijianyuyueModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await TijianyuyueModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM tijianyuyue WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM tijianyuyue WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM tijianyuyue WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM tijianyuyue WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM tijianyuyue WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM tijianyuyue WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM tijianyuyue " + where + " GROUP BY " + columnName
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("tijianyuyue" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM tijianyuyue " + where + " GROUP BY " + xColumnName 
	...
	api.get('/count', async (req, res) => {
		...
		try {
			let where = {}
			var tijianxiangmu = req.query.tijianxiangmu
			if (tijianxiangmu) {
			...
			const count = await TijianyuyueModel.count({where});
