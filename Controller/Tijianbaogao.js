...
import TijianbaogaoModel from '../models/TijianbaogaoModel'
...
export default ({ config, db }) => {
	...
	const renameMap = {
	    ...
	    "baogaoxiangqing": "baogaoxiangqing",
	    "baogaoshijian": "baogaoshijian",
	};
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let baogaoxiangqing = req.query.baogaoxiangqing
			if (baogaoxiangqing) {

				if (baogaoxiangqing.indexOf('%') != -1) {
					where.baogaoxiangqing = {
						[Op.like]: baogaoxiangqing
					}
				} else {
					where.baogaoxiangqing = {
						[Op.eq]: baogaoxiangqing
					}
				}
			}
			let baogaoshijian = req.query.baogaoshijian
			if (baogaoshijian) {

				if (baogaoshijian.indexOf('%') != -1) {
					where.baogaoshijian = {
						[Op.like]: baogaoshijian
					}
				} else {
					where.baogaoshijian = {
						[Op.eq]: baogaoshijian
			...
			let result = await TijianbaogaoModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await TijianbaogaoModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await TijianbaogaoModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await TijianbaogaoModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await TijianbaogaoModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await TijianbaogaoModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await TijianbaogaoModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM tijianbaogao WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM tijianbaogao WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM tijianbaogao WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM tijianbaogao WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM tijianbaogao WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM tijianbaogao WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM tijianbaogao " + where + " GROUP BY " + columnName
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("tijianbaogao" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM tijianbaogao " + where + " GROUP BY " + xColumnName 
