...
import YishengzuozhenModel from '../models/YishengzuozhenModel'
...
export default ({ config, db }) => {
	...
	const renameMap = {
	    ...
	    "zhicheng": "zhicheng",
	    "keshi": "keshi",
	    "shanzhanglingyu": "shanzhanglingyu",
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let zhicheng = req.query.zhicheng
			if (zhicheng) {

				if (zhicheng.indexOf('%') != -1) {
					where.zhicheng = {
						[Op.like]: zhicheng
					}
				} else {
			...
			let zuozhenshijian = req.query.zuozhenshijian
			if (zuozhenshijian) {

				if (zuozhenshijian.indexOf('%') != -1) {
					where.zuozhenshijian = {
						[Op.like]: zuozhenshijian
					}
				} else {
					where.zuozhenshijian = {
						[Op.eq]: zuozhenshijian
					}
				}
			}
			...
			let result = await YishengzuozhenModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await YishengzuozhenModel.findAll()
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await YishengzuozhenModel.findOne({where:dictionary})
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await YishengzuozhenModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await YishengzuozhenModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await YishengzuozhenModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await YishengzuozhenModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM yishengzuozhen WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM yishengzuozhen WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM yishengzuozhen WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM yishengzuozhen WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yishengzuozhen WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yishengzuozhen WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "yishengzuozhen"
			let where = " WHERE 1 = 1 "
			let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
			if(tableName == 'yiliaojigou') {
				where += " AND jigouzhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM yishengzuozhen " + where + " GROUP BY " + columnName
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("yishengzuozhen" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM yishengzuozhen " + where + " GROUP BY " + xColumnName 
