...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userQuery = await UsersModel.findOne({ where: { username: req.query.username} })
			if (userQuery!=null){
				if (userQuery.isLocked==1){
					toRes.session(res, 500, '账号已锁定，请联系管理员审核!')
				}
				if(pwd!=userQuery.password){//密码错误,累计错误次数
			...
			let userinfo = await UsersModel.findOne({ where: { username: req.query.username, password: pwd } })
			if (userinfo === null) {
			...
			userinfo.dataValues.loginUserColumn = 'username'
			userinfo.dataValues.tableName = 'users'
	...
	api.all('/session', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await UsersModel.findOne({ where: { id:jwt.decode(req.headers.token).id} }))
		} catch(err) {
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await UsersModel.findOne({where:dictionary})
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM users WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM users WHERE " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM users WHERE " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM users WHERE " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM users WHERE " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM users WHERE " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "users"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM users " + where + " GROUP BY " + columnName
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM users " + where + " GROUP BY " + xColumnName 
