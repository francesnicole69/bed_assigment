...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			let menujson = req.query.menujson
			if (menujson) {

				if (menujson.indexOf('%') != -1) {
					where.menujson = {
						[Op.like]: menujson
					}
				} else {
	...
	api.get('/query', async (req, res) => {
		...
		try {
			...
			let result = await MenuModel.findOne({where:dictionary})
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "menu"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM menu " + where + " GROUP BY " + columnName
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM menu " + where + " GROUP BY " + xColumnName 
